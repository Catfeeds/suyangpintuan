...
var main=function(){
    ...
    // fan 2016-05-10 start
    // 根据data-target 激活内部模态窗口
    modal:function(){
        var $this  = this._target,content,
            title = $this.getAttribute('data-title')||'弹窗',
            ...
            if (url){
                $.ajax({
                    ...
                    callback: function(res) {
                        if (res){
                            com.xshow(title, res, { hideBtn: true });
                        }
                    }
                });
            }else{
                ...
                content = $(target).html();
                com.xshow(title, content, { hideBtn: true });
            }
    ...
    // fan 2017-08-21 start
    // 给本元素 data-target 指定的对象 添加或者移除 data-class 指定的类名
    toggle:function(){
        var $this  = this._target,
            ...
            if (target && className) {
                var $target = $(target)
                if ($target.length && $target.hasClass(className)){
    ...
    // 给本元素 data-target 指定的对象 添加 data-class 指定的类名
    setTarget: function(){
        var $this  = this._target,
            target = $this.getAttribute('data-target'),
            className = $this.getAttribute('data-class');
    ...
    // 给本元素 data-target 指定的对象 移除 data-class 指定的类名
                    ...
                    $post({
                        ...
                        callback:function(x){
                            ...
                            if(n!==-1){
                                ...
                                // fan 2016-04-18
                                // 点击触发模态窗口的时候, 浏览器的当前url 会变成模态窗口的url
                                // 如果此时不关闭模态窗口. 直接刷新页面 将会造成背景主界面丢失
                                // 用 history.back(); 可以在模态窗口打开的时候,将浏览器的当前url 变回打开之前
                                // 但父类页面刷新,整个主控制器都重新执行了一次. 造成了完全不必要的性能损失
                                // todo 需要改变模态窗口的打开规则
                                // 最好的办法是:点击触发模态窗口的时候 原url不变, 同时也无需额外刷新页面
                                history.back();
                                // fan 2016-04-18 end
