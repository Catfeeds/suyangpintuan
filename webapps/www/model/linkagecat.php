...
class linkagecat_model extends Lowxp_Model {
	private $baseTable = '###_linkage_cat';
	...
	function loadCats($id = 0, $is_menu = 1) {
		$array = array();
		$data  = empty($options) ? $this->base->read_static_cache('linkagecats_' . $id . '_' . $is_menu, 'goodscat') : false;

		if ($data === false) {
			$array = $this->select($id);
			if (empty($options)) {
				$this->base->write_static_cache('linkagecats_' . $id . '_' . $is_menu, $array, 'goodscat');
			}
     ...
     * 载入商品分类（缓存）
     */
    function loadLinkageCats($tid = 0, $is_menu = 1) {
        $array = array();
        $data  = empty($options) ? $this->base->read_static_cache('linkagec_' . $tid . '_' . $is_menu, 'goodscat') : false;

        if ($data === false) {
            $catids = $this->db->getstr("select catid from ###_linkage where id={$tid}","catid");
            if($catids){
                $list = $this->db->select("select * from ". $this->baseTable ." where id in ({$catids}) ORDER BY listorder ASC,id ASC");
                foreach ($list as $v) {
                    $array[$v['id']] = $v;
                }
                if (!empty($list)) {
                    $this->base->write_static_cache('linkagec_' . $tid . '_' . $is_menu, $array, 'goodscat');
                }
            }
        } else {
            $array = $data;
     ...
     * 获取下级分类
     */
    function loadLinkageCatsByCid($tid = 0, $cid = 0) {
        $array = array();
        $data  = $this->loadLinkageCats($tid);
        foreach($data as $k=>$v){
