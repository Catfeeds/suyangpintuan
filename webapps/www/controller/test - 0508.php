...
class test extends Lowxp {
	...
	function score() {
		...
		$this->coupon->sendMany('353,314,313,363,316,232,121,124,123,353', 1, 1);
	}
...
//首次授权登录会带code参数，应该
	...
	function update() {
        ...
        function test(){            
            ...
            /*$domain['login_token'] = '25217,fc5f6326dda28ab4462e76bdfff6b3f1';
            $domain['format'] = 'json';
            $domain['domain'] = 'lnest.cc';
            $domain['sub_domain'] = 'testauth1';
            $domain['record_type'] = 'A';
            $domain['record_line'] = '默认';
            $domain['value'] = '59.56.68.83';
            $res = $this->curlPost($domain,'https://dnsapi.cn/Record.Create');
            if($res['status']['code']==1){*/
                $post['catid']=2;
                $post['type']=2;
                $post['vtype']='enter_one';
                $post['enter']=0;
                $post['expire']=date("Y-m-d H:i:s",RUN_TIME+3153600);//'2017-04-28 14:09:53';
                $post['title']='test';
                $post['authurl']='test.local';
                $post['vision']='1.0';
                $post['createtime']= date("Y-m-d H:i:s",RUN_TIME);//'2017-04-28 14:09:53';
                $post['status']='1';
                $res = curlPost(array('post'=>$post),'http://pin-auth.local/api/create_auth');
        ...
        function curlPost($curlPost, $url) {
            ...
            curl_setopt($ch, CURLOPT_USERAGENT, 'DNSPod API PHP Web Client/1.0.0 (yuyl@lnest.com)');
            $result = curl_exec($ch);
