...
class wheel extends Lowxp {
	...
	public function index($page = 1) {
		...
		$this->smarty->display('manage/wheel/list.html');
	...
	public function edit() {
		...
		$this->smarty->display('manage/wheel/edit.html');
	...
	public function editHandle($post) {
		...
		if (isset($res['error']) && $res['error'] == 0) {
			...
			$this->exeJs("parent.window.location='/manage#!wheel/index';");
		} else {
	...
	public function logs($page = 1) {
		...
		$this->smarty->display('manage/wheel/logs.html');
	}
	...
	public function detail() {
		...
		$this->smarty->assign('btnNo', 2);
		$this->smarty->display('manage/wheel/detail.html');
	}
	...
	public function dealExpress() {
		...
		$this->smarty->assign('row', $prize);
		$this->smarty->assign('btnNo', 2);
		$this->smarty->display('manage/wheel/dealExpress.html');
	...
	public function dealExpressHandle($post) {
		...
		$this->exeJs("parent.window.location='/manage#!wheel/logs';");
		exit;
	}
	...
	public function toggleActive() {
		...
		if ($id) {
			...
			$sql = 'UPDATE ' . $this->wheel->baseTable . ' SET status=not status , update_time = ' . RUN_TIME . ' WHERE id=' . $id;
			$this->db->query($sql);
			$res['error'] = 0;
			$res['msg']   = '操作成功';
		}
