...
class take_address extends Lowxp
{
    function __construct()
    {
        ...
        $this->btnMenu = array(
            ...
            1 => array('url' => '#!take_address/edit?com=xshow|自提地址添加', 'name' => '自提地址添加'),
    ...
    function index($page = 1)
    {
        ...
        $this->smarty->display('manage/take_address/index.html');
    }
    ...
    function edit($id = '')
    {
        ...
        $this->smarty->display('manage/take_address/edit.html');
    }
    ...
    function take_user($page=1){
        ...
        $this->smarty->display('manage/take_address/take_user.html');
    }
    /**
     * 添加核销人员
     */
    function take_user_add($id=0){
        ...
        $sql = "select * from `###_take_address` WHERE sid = 0";
        $data['list'] = $this->db->select($sql);
        $this->smarty->assign('data', $data);
        $this->smarty->display('manage/take_address/take_user_add.html');
    }
    /**
     * 删除核销人员
     */
    ...
    function ajax_user() {
        $mid = $_POST['mid'];
        if (empty($mid)) {
            die(json_encode(array('status' => 1, "msg" => "会员ID不能为空")));
        }

        $is_res = $this->db->get("select username,mobile from ###_member where mid={$mid}");
        if ($is_res == false) {
            die(json_encode(array('status' => 1, "msg" => "该会员不存在")));
        } else {
            die(json_encode(array('status' => 0, "msg" => "用户名：" . $is_res['username'], 'mobile' => $is_res['mobile'])));
        }
