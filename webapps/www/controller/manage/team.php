...
class team extends Lowxp
{
    ...
    function index($page = 1)
    {
        ...
        foreach ($list as $k => $v) {
            ...
            if ($v['status'] == 2) {//拼团失败
                ...
                if ($v['goods_typeid'] == CART_LUCK || $v['goods_typeid'] == CART_FREE) {//抽奖和免费试用 中奖的发货，没中奖的退款
                    if ($v['sid'] == 0) $v['ship_num'] = $this->db->getstr("select count(1) as num from ###_goods_order where common_id='{$v['id']}' and status_pay=10 and status_shipping<2 and status_lottery=10 and is_robots=0", "num");
                    ...
                    if ($v['sid'] == 0) $v['ship_num'] = $this->db->getstr("select count(1) as num from ###_goods_order where common_id='{$v['id']}' and status_pay=10 and status_shipping<2 and is_robots=0", "num");
                }
            }
        ...
        $this->smarty->display('manage/team/list.html');
    }
    ...
    function detail($id = '')
    {
        is_numeric($id) || die;
        $orderCommon = $this->order->orderCommon;
        $data = $this->team->get($id,'',array("where"=>" and a.is_robots=0 "));
        ...
        $this->smarty->display('manage/team/detail.html');
    }
    ...
    function refund()
    {
        ...
        $count = 0;
        $order_list = $this->db->select("select o.id,o.status_pay,o.order_amount,o.mid,o.order_sn,o.status_lottery,o.extension_code,o.money_paid,o.pay_id,p.out_trade_no,p.trade_no from ###_goods_order as o left join ###_pay_log as p on o.id=p.order_id  where o.common_id={$order_common['id']} and o.is_robots=0 and o.status_order=0 ");
        foreach ($order_list as $k => $v) {

            if ($v['status_pay'] > 0) {//已付款订单执行退款申请操作
    ...
    function refund_one()
    {
        ...
        $row = $this->db->get("select o.id,o.status_pay,p.order_amount,o.mid,o.order_sn,o.status_lottery,o.extension_code,o.money_paid,o.pay_id,p.out_trade_no,p.trade_no from ###_goods_order as o left join ###_pay_log as p on o.id=p.order_id  where o.id={$id} and p.order_type !=".PAY_BS);
        if ($row['status_pay'] > 0) {//已付款订单执行退款申请操作
            $res = $this->db->update("goods_order", array("status_order" => 2), array("id" => $row['id']));
            if ($res > 0) {
                //添加退款记录
                $this->load->model("refund");
                if($row['extension_code']==CART_STEP) {//阶梯团 退定金和尾款
                    $out_res = $this->db->select("select out_trade_no,trade_no,order_amount from ###_pay_log where order_id={$row['id']} and is_paid=1");
                    foreach ($out_res as $k => $v) {
                        ...
                        $temp['out_trade_no'] = $v['out_trade_no'];
                        $temp['trade_no'] = $v['trade_no'];
                        $temp['payment'] = $payment[$row['pay_id']];
                        $this->refund->addRefundLog($temp);
                    }
                }else{
                    $temp['order_id'] = $row['id'];
                    $temp['order_amount'] = $row['order_amount'];
                    $temp['out_trade_no'] = $row['out_trade_no'];
            ...
            $this->db->update("goods_order", array("status_order" => 5), array("id" => $row['id']));
        }
        $this->tip("退款提交成功！", array('inIframe' => true));
        $this->exeJs("parent.com.xhide();parent.main.refresh()");
