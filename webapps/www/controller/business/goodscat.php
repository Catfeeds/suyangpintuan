...
class goodscat extends Lowxp {
	...
	function index() {
		...
		foreach ($list as $r) {
			...
			$r['str_business'] .= "<a href='#!goodscat/edit/?parentid=" . $r['id'] . "&com=xshow|添加子分类' class='iconfont icon-a' title='添加子分类'>&#xe604;</a>";
			$r['str_business'] .= "<a href='#!goodscat/edit/?id=" . $r['id'] . "&com=xshow|编辑分类' class='iconfont icon-a' title='编辑分类'>&#xe603;</a>";

			$r['str_business'] .= "<a href='javascript:;' onclick=\"main.confirm_del('goodscat/del','" . $r['id'] . "')\" class='iconfont icon-a' title='删除'>&#xe606;</a>";

			if ($r['isrec'] == 1) {
				$r['isrec'] = "<a href='javascript:;' class='c-green' onclick='main.chang_status(\"" . $r['id'] . "\",\"goods_cat\",\"isrec\")' title='点击设为不推荐'>推荐</a>";
			} else {
				$r['isrec'] = "<a href='javascript:;' class='c-gray' onclick='main.chang_status(\"" . $r['id'] . "\",\"goods_cat\",\"isrec\")' title='点击设为显示'>普通</a>";
			}
		...
		$this->smarty->display('business/goodscat/list.html');
	}
	...
	function edit() {
		...
		if (isset($_POST['Submit'])) {
			...
			if (isset($res['code']) && $res['code'] == 0) {
				...
				//$this->exeJs("parent.location.href='/business#!goodscat/index'");
		...
		$row['thumb_rec']      = isset($row['thumb_rec']) ? $row['thumb_rec'] : '';
		$row['html_thumb']     = $this->form->files('thumb', $row['thumb']);
		$row['html_thumb_rec'] = $this->form->files('thumb_rec', $row['thumb_rec']);
		...
		if (!$id) {
			$row['id']  = 0;
			$row['url'] = $row['isrec'] = $row['title'] = $row['keywords'] = $row['description'] = '';
		} else {
			$row['title']       = isset($row['title']) ? $row['title'] : '';
			$row['keywords']    = isset($row['keywords']) ? $row['keywords'] : '';
			$row['description'] = isset($row['description']) ? $row['description'] : '';
		}
		$this->smarty->assign('row', $row);
		$this->smarty->display('business/goodscat/edit.html');
	}
	...
	function del($id) {
		...
		/*if(in_array($id,array(CATE_GIFT,CATE_PACK))){
			            $this->tip('该分类禁止删除！',array('type'=>1));die;
		*/
		...
		admin_log_sid('删除商品分类：' . $this->db->getstr("SELECT catname FROM ###_goods_cat WHERE id=" . $id));
