...
class order extends Lowxp {
	...
	function index($page = 1) {
		...
		$condition = 'WHERE is_robots=0 AND sid='.BID.' AND (go.status_common=0 || (go.status_common=10 && go.extension_code!=5 && go.extension_code!=6) || ( (go.extension_code=5 || go.extension_code=6) && go.status_lottery=10) ) ';
        if($_GET['lstype']=='shipping'){//待发货
            ...
            $condition =" WHERE is_robots=0 AND sid=" . BID . " and go.status_order in (2,3,4) ";
		...
		$this->smarty->display('business/order/list.html');
	}
	...
	function detail($id = '') {
        ...
        if($order['coupon_id_sid']){
            ...
            if($coupon_id)$order['coupon_amount_sid'] = $this->db->getstr("select amount from ###_coupon where id={$coupon_id_sid}");
        }
        //商家信息
        if($order['sid']>0){
		...
		$this->smarty->display('business/order/detail.html');
	}
	...
	function edit() {
		...
		#物流列表
		$express = $this->db->select("SELECT * FROM ###_express WHERE status=1 and sid=".BID." ORDER BY id ASC");

		$this->smarty->assign('express', $express);

		$this->smarty->assign('row', $row);

		$this->smarty->display('business/order/edit.html');
	}
	...
	function goods() {
		...
		$this->smarty->display('business/order/goods.html');
	}
	...
	function pack($id = 0, $page = 1) {
		...
		$this->smarty->display('business/order/pack.html');
	}
	...
	function receivePay($order_id = '') {
		...
		$this->smarty->display('business/order/state_receive_pay.html');
	}
	...
	function refund($order_id = '') {
		...
		$this->smarty->display('business/order/state_refund.html');
	}
	...
	function cancel($order_id = '') {
		...
		$this->smarty->display('business/order/state_order_cancel.html');
	}
	...
	function chPrice($order_id = '') {
		...
		$this->smarty->display('business/order/state_chprice.html');
	}
	...
	function send($order_id = '') {
		...
		$this->smarty->display('business/order/send.html');
	}
	...
	function send_pack($order_id = '', $qishu = 1) {
		...
		$this->smarty->display('business/order/send_pack.html');
	}
	...
	function verify_order_code($order_id = ''){
		...
		$this->smarty->assign('order_id', $order_id);
		$this->smarty->display('business/order/verify_order_code.html');
	}
	...
	function viewExpress($order_id) {
		...
		$this->smarty->display('business/order/view_express.html');
	...
	function order_income($page = 1) {
		...
		$this->smarty->display('business/order/income.html');
	}
    ...
    function excelOrderSend() {
        if (isset($_POST['leadExcel']) && !empty($_POST['leadExcel'])) {
            ...
            foreach ($data as $key => $val) {
                ...
                $order = $this->db->get('SELECT id,mid,order_sn,area,address,extension_code,common_id,status_shipping,sid FROM ###_goods_order where id = ' . $order_id);
                if($temp['id'] && $temp['express'] && $temp['express_num'] && $order['status_shipping']==0 && $order['sid']==BID){
                    ...
                    if($res!==false){
                        ...
                        $wxmsgParams = array(
                            "url" => note_url("/member/order_ship/".$order['order_sn']),
        ...
        $template = "business/order/order_send_excel.html";
