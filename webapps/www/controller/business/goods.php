...
class goods extends Lowxp {
    ...
    function __construct() {
        ...
        $this->btnMenu = array(
            ...
            2 => array('url' => '#!goods/uncheck', 'name' => '审核不通过'),
    ...
    function index($page = 1) {
        ...
        $this->smarty->display('business/goods/list.html');
    }
    //审核不通过商品列表
    function uncheck($page = 1){
        #检索
        $conds = $this->getCondtions();
        $condtions = $conds['where'];
        $orderby = $conds['order'];
        ...
        $sql = "SELECT * FROM ###_goods WHERE " . $condtions . " and is_check=2 ORDER BY " . $orderby;
        $data['list'] = $this->page->hashQuery($sql)->result_array();
        ...
        $this->smarty->assign('data', $data);
        $this->smarty->display('business/goods/uncheck.html');
    }
    function edit() {
        $this->load->model("business");
        $business = $this->business->get(BID);
        $this->smarty->assign('business', $business);
        //提交
        if (isset($_POST['Submit'])) {
            $_POST['post']['is_sale'] = 0;
            $_POST['post']['is_check'] = 0;

            //团长免单和试用的 限购一件
            if($_POST['post']['typeid']==CART_FREE || $_POST['post']['discount_type']==1){
                $_POST['post']['limit_buy_bumber'] = 1;
            }
            if($business['check_status']==1){
                //判断标题，推荐理由，内容是否含有敏感词
                $bw['name'] = $_POST['post']['name'];
                $bw['desc'] = $_POST['post']['desc'];
                $bw['content'] = $_POST['post']['content'];
                if(!isbadWord($bw)){
                    $_POST['post']['is_sale'] = 1;
                    $_POST['post']['is_check'] = 1;
                }
            }
            ...
            if (isset($res['code']) && $res['code'] == 0) {
                ...
                $this->exeJs("parent.window.location='/business#!goods/index';");
                if($res['type']=='update'){
                    admin_log_sid('更新商品：' . $_POST['post']['name']);
                }else{
                    admin_log_sid('添加商品：' . $_POST['post']['name']);
                }
            } else {
        ...
        $amount = $business['type']==1?(C('per_fee')+C('per_deposit')):(C('com_fee')+C('com_deposit'));
        if($business['pay_status']==0 && $amount>0){
            $this->tip("您还没有缴纳保证金", array('inIframe' => true));
            $this->exeJs("parent.window.location='/business#!goods/index';");
        }
        ...
        //编辑
        if ($id) {
            $row = $this->db->get("SELECT * FROM ###_goods WHERE id=" . $id." and sid=".BID);
        ...
        $this->smarty->assign('select_categorys', $select_categorys);*/
        //获取下拉同城分类
        $row['areaid'] = $business['zone'];
        $select_areacat = $this->area_option($business['zone'],$row);
        ...
        $coupon_list = $this->coupon->getAvailableCouponList(" and sid=".BID);
        ...
        #快递方式
        $row['express_list'] = $this->db->select("select * from ###_express where status = 1 AND sid = ".BID." order by listorder asc,id asc");
        #自提点
        $this->load->model("take_address");
        $row['take_address_list'] = $this->take_address->selectBySid(BID);
        #配送区域
        ...
        //商品类型
        $this->load->model("order");
        $this->smarty->assign('actTypes', $this->order->actTypes);

        $this->smarty->assign('row', $row);
        $row['cid'] = isset($row['cid']) ? $row['cid'] : 0;
        $this->smarty->display('business/goods/edit.html');
    ...
    function changSpec() {
        ...
        $this->smarty->display('business/goods/spec.html');
    }
    ...
    function selCid($cid, $id = 0) {
        ...
        if (!empty($sp_ids)) {
            ...
            $this->smarty->assign('spec_list', $speclist);
            $res = $this->smarty->fetch('business/goods/edit_spec.html');
        } else {
            $this->smarty->assign('spec_list', array());
            $res = array();
        }
    ...
    public function selection($page = 1) {
        ...
        $this->smarty->assign('data', $data);
        $this->smarty->display('business/goods/selection.html');
    }
    ...
    public function selection_wheel($page = 1) {
        ...
        $this->smarty->assign('only', $only);
        $this->smarty->assign('data', $data);
        $this->smarty->display('business/goods/selection_wheel.html');
    }
    ...
    function batch_price($page = 1) {
        ...
        $this->smarty->assign('data', $data);
        $this->smarty->display('business/goods/batch_price.html');
    }
    ...
    function batch_price_save() {
        ...
        $this->exeJs("parent.window.location='/business#!goods/batch_price?com=xshow|批量修改价格&cid={$_POST['cid']}';");
    }
    ...
    function excelGoods() {
        ...
        $template = "business/goods/goods_excel.html";
