...
include dirname(__FILE__) . '/RedisMessageQueue.php';

/**
 * 提供了写入位置和读取位置记录的功能 但是性能不如前者
 */
class RedisMessageQueueStatus extends RedisMessageQueue {
	...
	public function get() {
		if ($queue = parent::get()) {
			...
			if (!$incr_result) {
				throw new \Exception('can not mark get position,please check the redis server');
			}
	...
	public function put($message) {
		if (parent::put($message)) {
			...
			if (!$incr_result) {
				throw new \Exception('can not mark put position,please check the redis server');
			}
