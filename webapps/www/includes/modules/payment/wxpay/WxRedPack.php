...
class WxRedPack {
	...
	function __construct() {
		require_once AppDir . 'function/payment.php';
		$payment = get_payment('wxpay');

		if (isset($payment)) {

			$this->mchappid = $payment['wxpay_app_id'];

			$this->appsecret = $payment['wxpay_app_secret'];

			$this->mchid = $payment['wxpay_mchid'];

			$this->apikey = $payment['wxpay_key'];

			$this->apiclient_cert = $payment['cert_pem'];

			$this->apiclient_key = $payment['key_pem'];
