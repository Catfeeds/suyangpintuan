...
class AlipaySubmit {
	...
	function buildRequestForm($para_temp, $method, $button_name) {
		...
		// $sHtml = $sHtml . '<input  class="enter_s1 enter_w1" type=""submit"" value="' . $button_name . '"></form>';
		$sHtml .= '<input  id="alipaysubmit_button" class="enter_s1 enter_w1" type="button" value="' . $button_name . '"></form>';
		$sHtml .= '</form>';
		// $sHtml .= '<h3 style="color:red">因支付宝政策原因.无法直接在微信内部使用支付宝.</h3><h3 style="color:red">微信内部请点击右上角<span style="color:blue;">[在浏览器中打开]</span>完成支付</h3>';
		// $sHtml = $sHtml . "<script>document.forms['alipaysubmit'].submit();</script>";

		$sHtml .= <<<EOT
<script type="text/javascript">
		$(function() {
		    $('#alipaysubmit_button').on('click', function() {
		        $.getJSON('/welcome/respond/$para_temp[out_trade_no]', {
		                ajax: 1
		            },
		            function(data) {
		                if (data.is_success == 0) {
		                	...
		                	window.location.href="/welcome/respond/$order[log_id]";
		                }
		            });
		    });
		});
