...
if (!function_exists("phpFastCache")) {
	 ...
	 * @return mixed
	 */
	function phpFastCache($storage = '', $config = array()) {
...
class phpFastCache {
	...
	public static function getAutoClass($config) {
		// 这里判断有误 如果文件可写 则驱动为文件 除非指定 否则永远无法顺利自动调用 redis
		...
		// 考虑到 一般只使用 redis 跟file  所以其他的判断暂时注销 提高效率
		// 无配置默认使用file

		if (strtolower($config['storage']) == 'redis' && class_exists("Redis")) {
			$driver = "redis";
			// } elseif (extension_loaded('apc') && ini_get('apc.enabled') && strpos(PHP_SAPI,
			// 	"CGI") === false
			// ) {
			// 	$driver = "apc";
