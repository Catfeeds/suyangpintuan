...
class Smarty extends Smarty_Internal_TemplateBase {
    ...
    /**************************shh Layout*******************************/
    private $layout_tpl = false;  #layout模板名
    private $layout_content = 'LAYOUT_CONTENT';  #layout子模板变量
    private $layout_no = array("'LAYOUT_NO'","\"LAYOUT_NO\"");  #退出layout模板标签变量

    #配置layout
    public function setLayout($options){
        $this->layout_tpl = isset($options['layout_tpl']) ? $options['layout_tpl'] : $this->layout_tpl;
        $this->layout_content = isset($options['layout_content']) ? $options['layout_content'] : $this->layout_content;
        $this->layout_no = isset($options['layout_no']) ? $options['layout_no'] : $this->layout_no;
    }

    #重置display
    public function display($page, $cache_id = null, $compile_id = null, $parent = null) {
        if(!$this->templateExists($page)){
            //trigger_error("The template '$page' does not exist!");die;
            if (STPL == 'mobile') {
                die("请使用电脑访问");
            }else{
                die("请使用手机访问");
        ...
        if(!empty($this->layout_tpl)){
            $tpl = $this->createTemplate($page);
            $tag = $this->getTags($tpl);
            $tag = isset($tag[0][1][0]['var']) ? $tag[0][1][0]['var'] : '';

            #使用 layout 机制
            if(!in_array($tag,$this->layout_no)){
                $this->assign($this->layout_content, $page);
                $page = $this->layout_tpl;
            }
        }

        parent::display_base($page, $cache_id, $compile_id, $parent);
    }
    /**************************shh Layout end*******************************/
}
